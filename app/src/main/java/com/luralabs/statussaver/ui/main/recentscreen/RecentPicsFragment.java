...
import static com.luralabs.statussaver.ui.imageslider.ImageSliderActivity.EXTRA_IMAGE_TRANSITION_NAME;
...
//                presenter.saveMedia(adapter.getItemAtPosition(position));
        ...
        adapter.getOnStatusDeleteClicks().subscribe(new Action1<Integer>() {
            @Override
            public void call(Integer position) {
                presenter.confirmStatusDeleteAction(adapter.getItemAtPosition(position), position);
            }
        });
    ...
    @Override
    public void displayStatusDeleteSuccessMsg() {
        Snackbar.make(rootView, "Image removed from WhatsApp items", Snackbar.LENGTH_SHORT).show();
    }
    ...
    @Override
    public void displayStatusDeleteConfirmPrompt(final ImageModel imageModel, final int itemPosition) {
        String title = getString(R.string.title_delete_confirm_dialog);
        String msg = getString(R.string.msg_alert_status_delete_item_confirm);

        DialogFactory.createOKCancelDialog(getActivity(), title, msg, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        // Ok
                        presenter.deleteStatusImage(imageModel);
                        presenter.loadRecentImages();
                    }
                },
                new DialogInterface.OnClickListener() {

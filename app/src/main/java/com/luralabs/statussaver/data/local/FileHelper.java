package com.luralabs.statussaver.data.local;
...
import com.luralabs.statussaver.util.FileUtil;
...
public class FileHelper {
    ...
    public boolean deleteImageFromWhatsAppDir(ImageModel imageModel) {
        File file = new File(getWhatsappStatusDirPath(), imageModel.getFileName());
        if (file.exists()) {
            boolean status = file.delete();
            if (status) {
                mediaStateChangeSubject.onNext(imageModel);
